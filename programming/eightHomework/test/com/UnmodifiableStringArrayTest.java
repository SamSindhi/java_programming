...
public class UnmodifiableStringArrayTest {
    ...
    @Before
    public void init() { //because by design Before Class must be static
        if(usa == null) {
    ...
    public void test_Find() {
      assertEquals("USA", usa.find("USA"));
      assertEquals(null, usaFind("STR"));
    }

    @Test
    public void test_MaxLength() {
      assertEquals("Longest String", usa.maxLength());
    }

    @Test
    private void test_MinLength() {
      assertEquals("x", usa.minLength());
    }

   @Test
   private void test_Size() {
     assertEquals(6, usa.size());
   }
